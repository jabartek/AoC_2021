...
class Board {
 public:
  ...
  Board(I dim) : vertical_sums(dim, dim), horizontal_sums(dim, dim), total{0} {}
...
int main(int argc, const char *argv[]) {
  ...
  for (auto v : bingo_sequence) {
    for (auto &b : boards) {
      ...
      --b.vertical_sums[it->second.first];
      --b.horizontal_sums[it->second.second];
      b.total -= v;
      if (b.vertical_sums[it->second.first] == 0 ||
          b.horizontal_sums[it->second.second] == 0) {
        result.val = v;
